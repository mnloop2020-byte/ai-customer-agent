...
الخطوة: ${routing.nextAction}${hasObjection ? `\nالاعتراض: ${objectionType}` : ""}${missingFields.length ? `\nحقول مفقودة: ${missingFields.join(", ")}` : ""}${ctaButtons.length ? `\nCTA المقترح: ${ctaButtons.map(b => b.label).join(" | ")}` : ""}`;
  ...
  // ── 4. صيغة الرد الإلزامية ──
  const strictFormat = `[صيغة الرد الإلزامية — STRICT FORMAT]
يجب أن يكون الرد مكوّن من:

ابدأ بإقناع مرتبط بكلام العميل.
ثم نتيجة واضحة مرتبطة بالمشكلة.
ثم سؤال واحد فقط في النهاية.

ممنوع:
- كتابة أكثر من 3 جمل
- كتابة أكثر من سؤال
- شرح النظام أو الميزات
- ذكر الباقات أو الأسعار
- استخدام كلام عام مثل "تحسين تجربة العملاء"
- ممنوع كتابة أي عناوين أو أقواس أو تسميات مثل [نقطة] أو [سؤال] أو [إجابة مباشرة]

يجب كتابة الرد كنص طبيعي فقط وكأنك تتحدث مع العميل مباشرة.
اكتب الرد مباشرة بدون أي عناوين أو أقواس.

إذا خالف الرد هذه الصيغة → يعتبر الرد غير صالح`;

  // ── 5. عقد التنفيذ الإلزامي ──
